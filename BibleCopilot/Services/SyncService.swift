...
actor SyncService {
    ...
    private var authHeaders: [String: String] { AuthService.shared.authHeaders }
    ...
    private func pullSavedPassages(context: ModelContext) async {
        ...
        guard let data = await fetch(table: "saved_passages", filter: "user_id=eq.\(uid)") else { return }
    ...
    private func pullJournalEntries(context: ModelContext) async {
        ...
        guard let data = await fetch(table: "journal_entries", filter: "user_id=eq.\(uid)") else { return }
        guard let items = try? JSONSerialization.jsonObject(with: data) as? [[String: Any]] else { return }
    ...
    private func pullReadingPlanProgress(context: ModelContext) async {
        ...
        guard let data = await fetch(table: "reading_plan_progress", filter: "user_id=eq.\(uid)") else { return }
        guard let items = try? JSONSerialization.jsonObject(with: data) as? [[String: Any]] else { return }
    ...
    private func upsert(table: String, body: [String: Any], onConflict: String? = nil) async {
        ...
        var urlStr = "\(authService.supabaseURL)/rest/v1/\(table)"
        ...
        request.setValue(authService.supabaseKey, forHTTPHeaderField: "apikey")
